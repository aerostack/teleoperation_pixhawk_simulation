...
--wait drone_id_namespace:=drone$NUMID_DRONE fcu_url:=udp://:14540@192.168.1.36:14557" C-m

tmux new-window -t $SESSION:3 -n 'pixhawk interface'
tmux send-keys "roslaunch pixhawk_interface pixhawk_interface.launch \
--wait drone_id_namespace:=drone$NUMID_DRONE simulation_mode:=true " C-m
...
tmux new-window -t $SESSION:6 -n 'GroundTruth Gazebo'
...
#tmux send-keys "roslaunch thrust_controller_v2 thrust_controller_v2.launch \
#  namespace:=drone$NUMID_DRONE \
#  uav_mass:=$UAV_MASS" C-m
